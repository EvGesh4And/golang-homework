...
func (s *Server) getEventFromBody(ctx context.Context, r *http.Request) (storage.Event, error) {
	...
	s.logger.DebugContext(ctx, "request body parsed to event")
...
func (s *Server) getEventIDFromBody(ctx context.Context, r *http.Request) (uuid.UUID, error) {
	...
	s.logger.DebugContext(ctx, "attempting to extract event ID from request params")
	...
	s.logger.DebugContext(ctx, "event ID extracted from request params")
	return uuID, nil
}
